...
export const MentorManagement: React.FC = () => {
  ...
  const fetchMentors = async () => {
    ...
    try {
      ...
      setError(err.response?.data?.message || 'Failed to load mentors directory.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleStatusToggle = async (id: string, currentStatus: string) => {
    ...
    try {
      ...
      showToast(`Mentor status configured to ${nextStatus}.`, 'success');
      fetchMentors();
    } catch (err: any) {
      showToast(err.response?.data?.message || 'Failed to update mentor account status.', 'error');
    }
  };
  ...
  return (
    <div className="flex flex-col gap-6 animate-enter">
      <div className="flex flex-col gap-1">
        <h1 className="text-2xl font-bold font-display tracking-tight text-foreground">Mentors Directory</h1>
        <p className="text-sm text-muted-foreground">Manage expert credentials, assign classes, and inspect availabilities.</p>
      </div>
      ...
      {/* Grid listing */}
      {loading ? (
        <LoadingState message="Fetching mentors directory..." />
      ) : error ? (
        <ErrorState message={error} onRetry={fetchMentors} />
      ) : mentors.length === 0 ? (
        <EmptyState description="No mentor accounts matched your criteria." />
      ) : (
                ...
                <TableRow key={item.id}>
                  ...
                  <TableCell className="text-right flex items-center justify-end gap-2.5">
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={() => navigate(`/admin/mentors/${item.id}`)}
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-between p-2">
              <span className="text-xs text-muted-foreground">
                Showing <strong className="text-foreground">{mentors.length}</strong> of{' '}
                <strong className="text-foreground">{totalItems}</strong> mentor records
              </span>
